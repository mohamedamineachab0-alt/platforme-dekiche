...
1. نص السؤال: يجب أن يتضمن التحديد الدقيق للمعطيات والمطلوب بصيغة رياضية بحتة وباللغة المحددة (${langLabel}):
   ...
   - الهندسة الفضائية: في معلم متعامد ومتجانس $(O; \vec{i}, \vec{j}, \vec{k})$، شعاع ناظمي للمستوى $(P)$ ذي المعادلة $2x - 3y + z - 5 = 0$ هو:
   - الاحتمالات: إذا كان $P(A) = 0.4$ و $P(B) = 0.5$ والحدثان $A$ و $B$ مستقلان، فإن $P(A \cap B)$ يساوي:
   - الفيزياء (RC / RL / RLC): في دارة شحن مكثفة $(R, C)$ المغذاة بمولد قوته المحركة $E$، المعادلة التفاضلية التي يحققها التوتر $u_C(t)$ هي:
   - الكيمياء (أكسدة-إرجاع / أحماض-أسس): الثنائية $(\text{Ox} / \text{Red})$ الموافقة لشاردة البرمنغنات $\text{MnO}_4^-$ في وسط حمضي هي:
   - النووي: تفكك نواة اليورانيوم $^{238}_{92}\text{U}$ بنمط $\alpha$ يعطي نواة الثوريوم $^{A}_{Z}\text{Th}$، حيث:
   - الميكانيك: وفق القانون الثاني لنيوتن $\sum \vec{F}_{ext} = m \cdot \vec{a}$، تسارع مركز عطالة الجملة هو:

2. خيارات الإجابة الأربعة ('options'):
   - ممنوع منعاً باتاً كتابة خيارات كلامية أو نصوص سردية للمواد العلمية والرياضية!
   - كل خيار من الخيارات الأربعة يجب أن يكون معادلة رياضية أو قيمة علمية داخل علامتي دولار $...$:
     * دوال: ["$f'(x) = 6x - 5$", "$f'(x) = 3x^2 - 5$", "$f'(x) = 6x + 5$", "$f'(x) = 3x - 5$"]
     * نهايات: ["$\lim_{x \to +\infty} f(x) = 2$", "$\lim_{x \to +\infty} f(x) = 0$", "$\lim_{x \to +\infty} f(x) = +\infty$", "$\lim_{x \to +\infty} f(x) = -1$"]
     * متتاليات: ["$u_n = 4 \cdot 2^n - 3$", "$u_n = 2^n + 3$", "$u_n = 3 \cdot 2^n - 2$", "$u_n = 2^{n+1} - 1$"]
     * أعداد مركبة: ["$z = 2 e^{i \frac{5\pi}{6}}$", "$z = 2 e^{-i \frac{\pi}{6}}$", "$z = \sqrt{3} e^{i \frac{2\pi}{3}}$", "$z = 2 e^{i \frac{7\pi}{6}}$"]
     * كهرباء: ["$\tau = R \cdot C$", "$\tau = \frac{R}{C}$", "$\tau = \frac{L}{R}$", "$\tau = \frac{1}{R \cdot C}$"]
     * كيمياء: ["$\text{MnO}_4^- / \text{Mn}^{2+}$", "$\text{Mn}^{2+} / \text{MnO}_4^-$", "$\text{MnO}_4^{2-} / \text{Mn}^{2+}$", "$\text{MnO}_2 / \text{Mn}^{2+}$"]
