          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sticky top-6">
            ...
            <form action={async (formData) => { "use server"; await createReviewCard(formData); }} className="space-y-4">
              <div className="space-y-1">
                ...
                <input type="text" name="title" required className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-base focus:outline-none focus:ring-2 focus:ring-[#6D28D9]" placeholder="مثال: تعريف الخلية" />
              </div>

              <div className="space-y-1">
                <label className="text-sm font-bold text-slate-700">السؤال (الوجه الأمامي)</label>
                <textarea name="question" rows={3} required className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-base focus:outline-none focus:ring-2 focus:ring-[#6D28D9] resize-none" placeholder="اكتب السؤال هنا.." />
              </div>

              <div className="space-y-1">
                <label className="text-sm font-bold text-slate-700">الجواب (الوجه الخلفي)</label>
                <textarea name="answer" rows={3} required className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-base focus:outline-none focus:ring-2 focus:ring-[#6D28D9] resize-none" placeholder="اكتب الجواب هنا.." />
              </div>
              ...
              <div className="pt-2 border-t border-slate-100">
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="text" name="exerciseRef" className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-base focus:outline-none focus:ring-2 focus:ring-[#6D28D9]" placeholder="مثال: الوحدة الأولى - تمرين 4" />
                  </div>
                </div>
              </div>
                ...
                <div key={card.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 flex flex-col relative overflow-hidden group">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                      <Link href={`/dashboard/admin/review-cards/${card.id}/edit`} className="p-1.5 text-slate-400 hover:text-[#6D28D9] hover:bg-[#F3EFFF] rounded-lg transition-colors">
                  ...
                  <div className="space-y-3 mt-2 flex-1">
                    <div>
                      <span className="text-[10px] font-black text-[#A78BFA] uppercase tracking-wider mb-1 block">السؤال</span>
                      <p className="text-sm font-bold text-slate-700 bg-slate-50 p-3 rounded-xl border border-slate-100">{card.question}</p>
                    </div>
                    <div>
                      <span className="text-[10px] font-black text-[#6D28D9] uppercase tracking-wider mb-1 block">الجواب</span>
                      <p className="text-sm font-medium text-[#1E1B4B] bg-[#F3EFFF] p-3 rounded-xl border border-[#EDE9FE]">{card.answer}</p>
                    </div>
                  </div>
