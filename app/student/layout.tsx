  ...
  return (
    <div dir="rtl" className="min-h-screen flex flex-col md:flex-row relative z-10 font-sans">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-2 w-full max-w-[220px]">
               <span className="block w-full bg-white/10 rounded-full px-3 py-1.5 text-white text-[10px] font-medium whitespace-nowrap overflow-hidden text-ellipsis">النجاح ليس في بلوغ القمة بل الاستمرار في الصعود</span>
            </div>
          </div>
        </div>
              ...
              {sidebarLinks.map((link, idx) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 font-bold p-4 rounded-2xl transition-all ${isActive ? 'bg-[#6D28D9] text-white shadow-md shadow-[#6D28D9]/20' : 'text-slate-600 hover:bg-[#6D28D9]/10 hover:text-[#6D28D9]'
                      }`}
                  >
      ...
      <button
        ...
        className={`hidden md:flex fixed top-6 z-50 p-2.5 bg-white border border-gray-200 text-[#6D28D9] rounded-xl shadow-md hover:bg-gray-50 transition-all duration-300 ${isOpen ? "right-[300px]" : "right-6"
          }`}
      ...
      {/* --- Desktop Sidebar --- */}
      <div
        className={`hidden md:flex flex-col h-screen fixed right-0 top-0 bg-white shadow-[0_0_40px_rgba(0,0,0,0.04)] z-40 border-l border-gray-100 transition-all duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"
          } w-[320px]`}
      >
        {/* Sidebar Header */}
          ...
          {sidebarLinks.map((link, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={link.href}
                className={`flex items-center gap-3 px-4 font-bold py-4 rounded-2xl transition-all duration-300 ${isActive
