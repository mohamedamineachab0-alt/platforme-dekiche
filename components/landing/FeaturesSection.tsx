    ...
    <section id="features" className="scroll-mt-28 px-4 py-14 sm:px-6 sm:py-16 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <Reveal>
          ...
          <h2 className="max-w-xl text-3xl font-black text-[#1E1B4B] sm:text-4xl">دراسة منظّمة من البيت</h2>
            ...
            return (
              <Reveal key={feature.title} delay={Math.min(index * 70, 280)}>
                <Link
                  ...
                  className={`edu-skill-card block h-full rounded-[24px] border p-6 ${
                    feature.filled
                      ? "border-transparent bg-[#6D28D9] text-white"
                      : "border-[#EDE9FE] bg-white/90 text-[#1E1B4B]"
                  }`}
                >
                  <span
                  ...
                  <p
                    className={`mt-2 text-sm font-medium leading-relaxed ${
                      feature.filled ? "text-white/80" : "text-[#6B6480]"
                    }`}
                  >
                    {feature.desc}
                  </p>
...
function ParentIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" aria-hidden>
      ...
      <path
        d="M4.5 18c.8-3 2.6-4.5 4.5-4.5s3.7 1.5 4.5 4.5M13 18c.5-2 1.7-3 3-3s2.4 1 3 3"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
      />
