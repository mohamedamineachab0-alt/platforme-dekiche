    ...
    <section id="steps" className="scroll-mt-28 px-4 py-14 sm:px-6 sm:py-16 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <Reveal>
          ...
          <h2 className="max-w-xl text-3xl font-black text-[#1E1B4B] sm:text-4xl">ثلاث خطوات للبدء</h2>
        ...
        <div className="mt-9 grid gap-4 md:grid-cols-3">
          {STEPS.map((step, index) => (
            <Reveal key={step.n} delay={index * 80}>
              <article className="flex h-full flex-col rounded-[28px] border border-[#EDE9FE] bg-[#FFFEF8] p-6 sm:p-7">
                <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-[#6D28D9] text-lg font-black text-white">
                  {step.n}
                </span>
        ...
        <Reveal delay={280} className="mt-8">
          <Link
            ...
            className="inline-flex items-center justify-center rounded-full bg-[#6D28D9] px-7 py-3.5 text-sm font-black text-white transition hover:bg-[#1E1B4B]"
          >
            {isAuthenticated ? "ادخل فصلك" : "ابدأ الآن"}
          </Link>
        </Reveal>
      </div>
    </section>
